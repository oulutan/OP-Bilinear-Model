...
MODEL_CKPT = 'good_models/14_Smaller_Convs_2016-11-12_time_03-35.ckpt'
FINAL_CKPT = 'good_models/14_Smaller_Convs_2016-11-12_time_03-35.ckpt'
...
def seismic_features(seismic):
    with tf.variable_scope("Seismic_Feats"):
        ...
        # seismic_feats = slim.max_pool2d(snet, [16, 1], [8, 1])
...
def inference(images, seismic, dropout_keep):
    with tf.variable_scope("inference"):
        ...
        with tf.variable_scope("Fusion"):
            ...
            s_arranged = tf.transpose(tf.expand_dims(seismic_feats, 1), [0, 1, 3, 2, 4])  # has dims 256,1,1,19,20
            F2 = tf.tile(s_arranged, [1, output_shape[1], output_shape[2], 1, 1])
            ...
            tf.add_to_collection('feats', [seismic_feats, image_feats, F2, F1, spatio_temporal_feats])
            ...
            c3_p = tflearn.max_pool_3d(c3_1, kernel_size=[1, 2, 2, 5, 1], strides=[1, 2, 2, 5, 1])
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        if not FLAGS.isTest:
            ...
            plt.savefig('prec_recall_bilinear.png')
