...
# vehicle test
# VALIDATION_FILE = 'avgOF_seismic_D2_vehicles.tfrecords'

MODEL_CKPT = 'good_models/01_Seismic_2016-11-13_time_08-50_pooled.ckpt'
# FINAL_CKPT = 'good_models/01_Seismic_2016-11-13_time_08-50_pooled.ckpt'
POOLED = False
FINAL_CKPT_wPOOLING = 'good_models/01_Seismic_2016-11-13_time_08-50_pooled.ckpt'
FINAL_CKPT_woPOOLING = 'good_models/01_Seismic_2016-11-13_time_09-48.ckpt'
# FINAL_CKPT_woPOOLING = 'good_models/01_Seismic_2016-11-13_time_08-50_pooled.ckpt'
FINAL_CKPT = FINAL_CKPT_wPOOLING if POOLED else FINAL_CKPT_woPOOLING
...
def read_and_decode(filename_queue):
    ...
    # Dont forget to normalize and scale to [0, 1]
...
def preprocess(labels, images, seismics):
    ...
    # mean_s, var_s = tf.nn.moments(seismics,axes=[1])
    # std_s = tf.sqrt(var_s)
    # white_seis = (seismics-tf.tile(tf.expand_dims(mean_s,1),[1, 4096]))/tf.tile(tf.expand_dims(std_s,1),[1, 4096])
    # p_seismics = tf.expand_dims(tf.expand_dims(white_seis, 2), 3)
...
def classification(final_feats, dropout_keep):
    with tf.variable_scope("classification"):
        ...
        # fnet = final_feats
        with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(scale=0.00005)):
           fnet = slim.fully_connected(final_feats, 32, weights_initializer=tf.truncated_normal_initializer(stddev=0.1))
...
def inference(images, seismic, dropout_keep):
    with tf.variable_scope("inference"):
        ...
        # Fusion
        final_feats = slim.flatten(seismic_feats)
...
def loss_op(y_probs, labels):
    ...
    with tf.name_scope("Regularizers"):
        ...
        regularizers = regularizers_FC + regularizers_SCNN
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        print([v.name for v in tf.get_collection(tf.GraphKeys.VARIABLES, scope="inference/classification")])
        ...
        seis_cnn_dict = {v.op.name: v for v in seis_cnn_vars}
