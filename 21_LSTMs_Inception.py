...
flags.DEFINE_integer('max_steps', 20000, 'Total number of steps to train')
...
TRAIN_FILE_1 = 'avgOF_seismic_D2_1P_train_norm_th15_1_LSTM.tfrecords'
TRAIN_FILE_2 = 'avgOF_seismic_D2_1P_train_norm_th15_2_LSTM.tfrecords'
VALIDATION_FILE = 'avgOF_seismic_D2_1P_test_norm_th15_LSTM.tfrecords'

# IMAGE_FEATS_CKPT = 'inception_v3_model/inception_v3.ckpt'
IMAGE_FEATS_CKPT = 'good_models/14_Smaller_Convs_2016-11-12_time_03-35.ckpt'

MODEL_CKPT = 'good_models/04_Images_Inception_2016-11-12_time_02-12_pooled.ckpt'
# FINAL_CKPT = 'good_models/04_Images_Inception_2016-11-12_time_02-12_pooled.ckpt'

POOLED = False
FINAL_CKPT_wPOOLING = 'good_models/04_Images_Inception_2016-11-13_time_00-44.ckpt'
FINAL_CKPT_woPOOLING = 'good_models/04_Images_Inception_2016-11-11_time_19-00.ckpt'
# FINAL_CKPT_woPOOLING = 'good_models/01_Seismic_2016-11-13_time_08-50_pooled.ckpt'
FINAL_CKPT = 'model_weights/21_LSTMs_Inception_2017-03-07_time_18-05.ckpt'
...
def read_and_decode(filename_queue):
    ...
    features = tf.parse_single_example(
      ...
      features={
            'image': tf.FixedLenFeature([100*100*NUM_STACK], tf.int64),
    ...
    image = tf.reshape(tf.cast(features['image'], tf.uint8), [100, 100, NUM_STACK])
...
def preprocess(labels, images, seismics):
    ...
    tiled = tf.tile(tf.expand_dims(interpolated,4), [1, 1, 1, 1, 3])
    p_images = (tiled / 256.0 - 0.5) * 2.0

    # tiled = tf.tile(images, [1, 1, 1, 3])
    # p_images = (tf.cast(tiled, tf.float32) / 256.0 - 0.5) * 2.0
...
def image_features(images):
    with tf.variable_scope("Image_Feats"):
        ...
        # ipdb.set_trace()
            ...
            image_feats = slim.max_pool2d(incp_feats, [4, 4], [2, 2])  # Bx7x7x192
            vis_feats =  image_feats
    return vis_feats
...
def seismic_features(seismic):
    with tf.variable_scope("Seismic_Feats"):
        ...
        snet = slim.conv2d(seismic, 16, [10, 1])
        snet = slim.conv2d(snet, 16, [10, 1])
        snet = slim.max_pool2d(snet, [2, 1], stride=[2, 1])
...
def classification(final_feats, dropout_keep):
    with tf.variable_scope("classification"):
        ...
        # fnet = final_feats
        with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(scale=0.00001)):
            fnet = slim.fully_connected(final_feats, 20, weights_initializer=tf.truncated_normal_initializer(stddev=0.1))
        fnet = slim.dropout(final_feats, dropout_keep)
...
def inference(images, seismic, dropout_keep):
    with tf.variable_scope("inference"):
        # Images is [batch_size, 150, 150, NUM_STACK, 3]
        # ipdb.set_trace()
        input_shape = images.shape.as_list()
        
        # permute so that shape is [NUM_STACK, batch_size, 150,150,3]
        # first dimension must be NUM_STACK so that split in RNN would work properly
        permuted_images = tf.transpose(images,[3,0,1,2,4])
        # Stack together everything for processing so it becomes[batch_size*NUM_STACK, 150,150,3]
        stacked_images = tf.reshape(permuted_images, [-1, input_shape[1], input_shape[2], input_shape[4]])

        # Image Model
        image_feats = image_features(stacked_images)

        # Seismic Model
        seismic_feats = seismic_features(seismic)

        # RNN
        BatchList = tf.split(slim.flatten(image_feats), NUM_STACK, axis=0)
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(64, forget_bias=1.0)

        output_feats, state = tf.contrib.rnn.static_rnn(lstm_cell, BatchList, dtype=tf.float32)

        # ipdb.set_trace()

        # Fusion
        final_feats = output_feats[-1]
...
def loss_op(y_probs, labels):
    ...
    with tf.name_scope("Regularizers"):
        ...
        regularizers = regularizers_inception + regularizers_FC
    # ipdb.set_trace()
...
def training(loss, learning_rate=1e-4):
    ...
    with tf.name_scope("ADAM_optimizer"):
        train_step = tf.train.AdamOptimizer(learning_rate=learning_rate, epsilon=1e-4).minimize(loss)
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        labels_tr, images_tr, seismics_tr, cam_locs_tr, ex_idxs_tr = inputs(train=True, batch_size=FLAGS.batch_size, num_epochs=FLAGS.num_epochs)
        ...
        # Input images and labels.
        labels_tst, images_tst, seismics_tst, cam_locs_tst, ex_idxs_tst = inputs(train=False, batch_size=FLAGS.batch_size,
